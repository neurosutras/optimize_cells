...
def run_tests():
    ...
    # Stage 1: Run simulation with a predefined amplitude of step current injections to the soma
    args = context.interface.execute(get_args_dynamic_single_adaptation, context.x0_array, features)
    group_size = len(args[0])
    sequences = [[context.x0_array] * group_size] + args + [[model_id] * group_size] + \
                [[context.export] * group_size] + [[context.plot] * group_size]
    primitives = context.interface.map(compute_single_adaptation, *sequences)
    sa_features = {key: value for feature_dict in primitives for key, value in viewitems(feature_dict)}
...
#                      12, 14, 16, 18, 20, 22, 23, 25, 27, 29, 30, 32, 33])/ 0.6  # Hz, from 600 ms current injections
    exp_i_inj_amp_f_I_0 = np.array([274.8677669, 299.4976432, 315.877205 , 327.2176965,
       337.9247978, 348.6333755])/1000 #nA
    exp_rate_f_I_0 = np.array([1.70804989, 4.03011188, 11.96446961, 17.33167727,
       22.93289975, 28.30047652        ])
    ...
    exp_i_inj_amp_spike_adaptation_0 = [0.1, 0.35 ]
    # last ISI / first ISI (%)
...
#                                    180.6451613, 193.5483871, 194.0860215]
    ...
    exp_spike_adaptation_array_0 = [121.4, 150]
...
def get_args_dynamic_single_adaptation(x, features):
    """
    A nested map operation is required to compute single adaptation ISI features. The arguments to be mapped depend on prior features
    (dynamic).
    :param x: array
    :param features: dict
    :return: list of list
    ...
    if 'i_holding' not in features:
        ...
        i_holding = features['i_holding']
    spike_detector_delay = features['spike_detector_delay']
    exp_freq_inj = context.I_freq_tsa

    return [[i_holding], [spike_detector_delay], [exp_freq_inj]]
...
def compute_single_adaptation(x, i_holding, spike_detector_delay, target_I_inj, model_id=None,
                        export=False, plot=False):
    """
    ...
    spike_times = np.array(context.cell.spike_detector.get_recordvec())
    indexes = np.where((spike_times > equilibrate) & (spike_times < equilibrate + stim_dur))[0]
    spike_times = spike_times[indexes]

    result = dict()
    result['sa_spike_times'] = spike_times
    vm = np.array(soma_rec)
...
#    spike_rate = spike_times.size / stim_dur * 1000.
#    result['spike_rate'] = spike_rate

    if context.verbose > 0:
        print('compute_single_adaptation: pid: %i; model_id: %s; %s: %s took %.1f s' %
...
# single adaptation
    if 'sa_spike_times' in current_features:
        target_spike_times = context.sa_spike_times_target 
...
##### Single Adaptation objective test
    target_spike_times = context.sa_spike_times_target 
    target_spk_cnt = target_spike_times.size
    target_ISI = np.diff(target_spike_times)
    lower_target_ISI = target_ISI * (1-context.ISI_tol_ratio)
    upper_target_ISI = target_ISI * (1+context.ISI_tol_ratio)
    ...
    err_vec = target_spike_times[all_idx] - model_spike_times[all_idx]
    ISI0diff = np.abs(model_spike_times[0] -  target_spike_times[0])
    ISI0sqerr = 0 if ISI0diff < context.ISI_tol_ratio*target_spike_times[1] else ISI0diff**2 
    sa_spike_times_residuals = np.dot(err_vec, err_vec) + ISI0sqerr 
    del features['sa_spike_times']
######
