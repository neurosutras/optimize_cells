...
parser.add_argument('plot', nargs='?', type=int, default=1, help='1 to plot, 0 ow')
...
filt_sig, env, env_ratio, centroid_freq, freq_tuning_index = \
    get_bandpass_filtered_signal_stats(carrier, time, sos, filter_band)
...
def FM(time, sos, filter_band, time_x, x0, fm, fc, m):
    ...
    filt_sig, env, env_ratio, centroid_freq, freq_tuning_index = \
                get_bandpass_filtered_signal_stats(signal, time, sos, filter_band)
...
normal(sos, time, filter_band, carrier, x1, mu=.6)
