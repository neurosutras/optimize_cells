...
#define nrn_init _nrn_init__LinExp2Syn
#define _nrn_initial _nrn_initial__LinExp2Syn
#define nrn_cur _nrn_cur__LinExp2Syn
#define _nrn_current _nrn_current__LinExp2Syn
#define nrn_jacob _nrn_jacob__LinExp2Syn
#define nrn_state _nrn_state__LinExp2Syn
#define _net_receive _net_receive__LinExp2Syn 
#define state state__LinExp2Syn 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 2;
 	...
 	ivoc_help("help ?1 LinExp2Syn /Users/wyin/optimize_cells/x86_64/lin_exp2syn.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Shared synaptic conductance with per-stream linear summation";
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _tsav = t; {
    ...
    double __state = A;
    double __primary = (A + _args[0] * _args[1] * factor) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau_rise ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau_rise ) - __primary );
    A += __primary;
  } else {
 A = A + _args[0] * _args[1] * factor ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B;
